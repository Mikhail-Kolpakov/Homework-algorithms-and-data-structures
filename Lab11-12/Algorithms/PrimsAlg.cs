...
public static class PrimsAlg
{
    ...
    private static int MinKey(int[] key, bool[] included, int verticesCounts) // Метод для знаходження вершини з мінімальним ключем серед тих, які ще не включені в основне дерево
    {
        ...
        {
            if (included[i] == false && key[i] < min) // Перевірка чи вершина ще не включена в остовне дерево та її ключ менший за поточний мінімум
            {
                min = key[i];
                minIndex = i;
            }
